...
<head>
    ...
    /* Admin Attachments Upload Styles */
    ...
    /* Admin Attachments Display in Notifications */
    .admin-attachments-display {
        margin-top: 15px;
        padding: 15px;
        background: #f8f9fa;
    ...
    .admin-attachments-display .attachments-header {
        font-weight: 600;
        color: #0d6efd;
        margin-bottom: 10px;
        display: flex;
    ...
    .admin-attachment-item .attachment-overlay {
        position: absolute;
        bottom: 0;
        left: 0;
        right: 0;
...
<div class="reporting-container">
                ...
                <div class="report-section">
                    ...
                    <div class="section-content">
                        ...
                        <ul class="report-list" id="mangrove-reports-list">
                            ...
                            // Add condition for Barangay Officials
                            if (!$isAdminOrRep && isset($city_municipality)) {
                                $mangroveSql .= " WHERE m.city_municipality = '" . $connection->real_escape_string($city_municipality) . "' ";
                            }

                            $mangroveSql .= "ORDER BY m.report_date DESC LIMIT 8";
                            $mangroveResult = $connection->query($mangroveSql);
                            
                            // Get total count for pagination
                            $mangroveCountSql = "SELECT COUNT(*) as total 
                                                FROM mangrovereporttbl m ";
                            if (!$isAdminOrRep && isset($city_municipality)) {
                                $mangroveCountSql .= " WHERE m.city_municipality = '" . $connection->real_escape_string($city_municipality) . "' ";
                            }
                            $mangroveCountResult = $connection->query($mangroveCountSql);
                            $mangroveTotal = $mangroveCountResult->fetch_assoc()['total'];
                        ...
                        <?php if ($mangroveTotal > 8): ?>
                        <div class="pagination-container" data-report-type="mangrove">
                            <button class="page-btn prev-btn" disabled>
                                <i class="fas fa-chevron-left"></i> Previous
                            </button>
                            <span class="page-info">Page <span class="current-page">1</span> of <span class="total-pages"><?= ceil($mangroveTotal / 8) ?></span></span>
                            <button class="page-btn next-btn" <?= ($mangroveTotal <= 8) ? 'disabled' : '' ?>>
                    ...
                    <div class="section-content">
                        ...
                        <div class="priority-section">
                            ...
                            <ul class="report-list" id="illegal-emergency-list">
                                ...
                                // Get total count for emergency pagination
                                $emergencyCountSql = "SELECT COUNT(*) as total 
                                                     FROM illegalreportstbl 
                                                     WHERE priority = 'Emergency' ";
                                if (!$isAdminOrRep && isset($city_municipality)) {
                                    $emergencyCountSql .= " AND city_municipality = '" . $connection->real_escape_string($city_municipality) . "' ";
                                }
                                $emergencyCountResult = $connection->query($emergencyCountSql);
                                $emergencyTotal = $emergencyCountResult->fetch_assoc()['total'];
                            ...
                            <?php if ($emergencyTotal > 3): ?>
                            <div class="pagination-container" data-report-type="illegal-emergency">
                                <button class="page-btn prev-btn" disabled>
                                    <i class="fas fa-chevron-left"></i> Previous
                                </button>
                                <span class="page-info">Page <span class="current-page">1</span> of <span class="total-pages"><?= ceil($emergencyTotal / 3) ?></span></span>
                                <button class="page-btn next-btn" <?= ($emergencyTotal <= 3) ? 'disabled' : '' ?>>
                                    Next <i class="fas fa-chevron-right"></i>
                                </button>
                            </div>
                            <?php endif; ?>
                        ...
                        <ul class="report-list" id="illegal-reports-list">
                            ...
                            $illegalSql .= "ORDER BY report_date DESC LIMIT 5";
                            $illegalResult = $connection->query($illegalSql);
                            
                            // Get total count for illegal pagination
                            $illegalCountSql = "SELECT COUNT(*) as total 
                                               FROM illegalreportstbl ir ";
                            if (!$isAdminOrRep && isset($city_municipality)) {
                                $illegalCountSql .= " WHERE ir.city_municipality = '" . $connection->real_escape_string($city_municipality) . "' ";
                            }
                            $illegalCountResult = $connection->query($illegalCountSql);
                            $illegalTotal = $illegalCountResult->fetch_assoc()['total'];
                        ...
                        <?php if ($illegalTotal > 5): ?>
                        <div class="pagination-container" data-report-type="illegal">
                            <button class="page-btn prev-btn" disabled>
                                <i class="fas fa-chevron-left"></i> Previous
                            </button>
                            <span class="page-info">Page <span class="current-page">1</span> of <span class="total-pages"><?= ceil($illegalTotal / 5) ?></span></span>
                            <button class="page-btn next-btn" <?= ($illegalTotal <= 5) ? 'disabled' : '' ?>>
                                Next <i class="fas fa-chevron-right"></i>
                            </button>
                        </div>
                        <?php endif; ?>
                ...
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body">
                        <form id="statusUpdateForm">
                            ...
                            <!-- Admin Documentation Attachments Section -->
                            <div class="mb-3" id="adminAttachmentsSection" style="display: none;">
                                <label class="form-label">
                                    <i class="fas fa-paperclip"></i> Admin Documentation (Optional)
                                    <small class="text-muted">Attach proof of investigation/action (Images or Videos)</small>
                                </label>
                                
                                <div class="attachment-upload-area" id="attachmentUploadArea">
                                    <input type="file" id="adminAttachments" name="admin_attachments[]" 
                                           accept="image/jpeg,image/png,image/gif,image/webp,video/mp4,video/avi,video/mov,video/wmv" 
                                           multiple style="display: none;">
                                    
                                    <div class="upload-prompt" onclick="document.getElementById('adminAttachments').click()">
                                        <i class="fas fa-cloud-upload-alt fa-3x text-primary"></i>
                                        <p class="mb-1"><strong>Click to upload</strong> or drag and drop</p>
                                        <small class="text-muted">Images (JPG, PNG, GIF, WEBP) or Videos (MP4, AVI, MOV, WMV)</small>
                                        <small class="text-muted d-block">Max 5 files, 10MB each</small>
                                    </div>
                                </div>
                                
                                <div id="attachmentPreviewContainer" class="attachment-preview-container mt-3"></div>
                                
                                <div id="attachmentWarning" class="alert alert-warning mt-2" style="display: none;">
                                    <i class="fas fa-exclamation-triangle"></i> <span id="attachmentWarningText"></span>
                                </div>
                            </div>
                        </form>
                    </div>
        ...
        <!-- Lightbox Modal for Admin Attachments -->
        <div id="adminImageLightbox" class="lightbox-modal">
            <span class="lightbox-close" onclick="closeAdminLightbox()">&times;</span>
            <div class="lightbox-content">
                <img id="adminLightboxImage" src="" alt="Admin Attachment">
                <div class="lightbox-controls">
                    <button id="adminPrevImage" class="lightbox-btn" onclick="prevAdminImage()"><i class='bx bx-chevron-left'></i></button>
                    <span id="adminImageCounter" class="image-counter"></span>
                    <button id="adminNextImage" class="lightbox-btn" onclick="nextAdminImage()"><i class='bx bx-chevron-right'></i></button>
                </div>
                <div class="lightbox-caption" id="adminLightboxCaption"></div>
            </div>
        </div>
        
        <!-- Video Player Modal for Admin Attachments -->
        <div id="adminVideoPlayerModal" class="lightbox-modal">
            <span class="video-close" onclick="closeAdminVideo()">&times;</span>
            <div class="lightbox-content">
                <video id="adminVideoPlayer" controls>
                    <source src="" type="video/mp4">
                    Your browser does not support the video tag.
                </video>
                <div class="lightbox-caption" id="adminVideoCaption"></div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const paginationState = {
        'mangrove': { currentPage: 1, limit: 8 },
        'illegal-emergency': { currentPage: 1, limit: 3 },
        'illegal': { currentPage: 1, limit: 5 }
    };
    ...
    document.addEventListener('click', async function(e) {
        ...
        // Update page number
        if (isPrev) {
            paginationState[reportType].currentPage--;
        } else {
            paginationState[reportType].currentPage++;
        }
    ...
    async function loadReports(reportType) {
        const state = paginationState[reportType];
        const listId = reportType === 'mangrove' ? 'mangrove-reports-list' : 
                      reportType === 'illegal-emergency' ? 'illegal-emergency-list' : 
                      'illegal-reports-list';
        
        const list = document.getElementById(listId);
        const container = document.querySelector(`.pagination-container[data-report-type="${reportType}"]`);
        
        if (!list || !container) return;
        
        // Show loading state
        list.innerHTML = '<li class="report-item"><div class="text-center"><i class="fas fa-spinner fa-spin"></i> Loading reports...</div></li>';
        
        try {
            const response = await fetch(`load_paginated_reports.php?report_type=${reportType}&page=${state.currentPage}&limit=${state.limit}`);
            ...
            if (data.success) {
                ...
                list.innerHTML = '<li class="report-item no-results">Error loading reports: ' + data.message + '</li>';
            }
        } catch (error) {
            console.error('Error loading reports:', error);
            list.innerHTML = '<li class="report-item no-results">Error loading reports. Please try again.</li>';
        }
    }
...
reportTypeFilter.addEventListener('change', filterStatusReports);
    ...
    // Handle status select change to show/hide attachments section
    document.getElementById('statusSelect').addEventListener('change', function() {
        const selectedStatus = this.value;
        const attachmentsSection = document.getElementById('adminAttachmentsSection');
        const attachmentInput = document.getElementById('adminAttachments');
        
        // Show attachments section only for Investigating, Action Taken, and Resolved
        const statusesWithAttachments = ['Investigating', 'Action Taken', 'Resolved'];
        
        if (statusesWithAttachments.includes(selectedStatus)) {
    ...
    // Admin attachments file handling
    ...
    const attachmentWarningText = document.getElementById('attachmentWarningText');
    
    // File input change handler
    adminAttachmentsInput.addEventListener('change', function() {
    ...
    attachmentUploadArea.addEventListener('dragleave', function(e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    function handleAdminFiles(files) {
        ...
        if (selectedAdminFiles.length + filesArray.length > MAX_FILES) {
            attachmentWarningText.textContent = `Maximum ${MAX_FILES} files allowed. You can select ${MAX_FILES - selectedAdminFiles.length} more file(s).`;
        ...
        for (const file of filesArray) {
            ...
            if (file.size > MAX_FILE_SIZE) {
                attachmentWarningText.textContent = `File "${file.name}" exceeds 10MB limit.`;
                attachmentWarning.style.display = 'block';
                continue;
            }
            ...
            if (!validTypes.includes(file.type)) {
                attachmentWarningText.textContent = `File "${file.name}" has invalid type. Only images and videos allowed.`;
                attachmentWarning.style.display = 'block';
                continue;
            }
    ...
    document.getElementById('submitStatusUpdate').addEventListener('click', async function() {
        ...
        try {
            ...
            formData.append('account_id', reportInfo.account_id);
            formData.append('action_type', newStatus);
            formData.append('admin_notif_description', generateAdminDescription(newStatus, userRole));
            formData.append('notif_description', actionDesc);
            formData.append('notifier_type', userRole === 'Barangay Official' ? 'accountstbl' : 'adminaccountstbl');
    ...
    document.getElementById('statusUpdateModal').addEventListener('hidden.bs.modal', function() {
        ...
        document.getElementById('statusUpdateForm').classList.remove('was-validated');
        ...
        document.getElementById('adminAttachmentsSection').style.display = 'none';
        
        // Reset button state
        const submitBtn = document.getElementById('submitStatusUpdate');
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<span id="submitStatusText">Update Status</span><span id="submitStatusSpinner" class="spinner-border spinner-border-sm d-none" role="status"></span>';
    });
    ...
    async function loadReportDetails(reportId, isMangrove) {
        try {
            ...
            console.log('Is Mangrove:', isMangrove);
            ...
            console.log('History Items Count:', historyData.history ? historyData.history.length : 0);
            ...
            if (isMangrove) {
                html += `
                    ...
                    ${(report.forest_cover_percent !== null && report.forest_cover_percent !== undefined) ? `
                    <div class="report-details-row">
                        <div class="report-details-label"><i class="fas fa-tree"></i> Forest Cover:</div>
                        <div class="report-details-value">
                            <strong>${parseFloat(report.forest_cover_percent).toFixed(1)}%</strong>
                            <div style="width: 100%; height: 8px; background: #e0e0e0; border-radius: 4px; margin-top: 6px; overflow: hidden;">
                                <div style="width: ${report.forest_cover_percent}%; height: 100%; background: linear-gradient(90deg, #4CAF50, #66BB6A); border-radius: 4px;"></div>
                            </div>
                        </div>
                    </div>
                    ` : ''}
                    ${(report.canopy_density_percent !== null && report.canopy_density_percent !== undefined) ? `
                    <div class="report-details-row">
                        <div class="report-details-label"><i class="fas fa-cloud-sun"></i> Canopy Density:</div>
                        <div class="report-details-value">
                            <strong>${parseFloat(report.canopy_density_percent).toFixed(1)}%</strong>
                            <div style="width: 100%; height: 8px; background: #e0e0e0; border-radius: 4px; margin-top: 6px; overflow: hidden;">
                                <div style="width: ${report.canopy_density_percent}%; height: 100%; background: linear-gradient(90deg, #2196F3, #42A5F5); border-radius: 4px;"></div>
                            </div>
                        </div>
                    </div>
                    ` : ''}
                    ${(report.tree_count !== null && report.tree_count !== undefined) ? `
                    <div class="report-details-row">
                        <div class="report-details-label"><i class="fas fa-calculator"></i> Trees Counted:</div>
                        <div class="report-details-value"><strong>${report.tree_count}</strong> trees in ${report.area_m2} m²</div>
                    </div>
                    ` : ''}
                    ${(report.calculated_density !== null && report.calculated_density !== undefined) ? `
                    <div class="report-details-row">
                        <div class="report-details-label"><i class="fas fa-chart-line"></i> Tree Density:</div>
                        <div class="report-details-value">
                            <strong>${parseFloat(report.calculated_density).toLocaleString('en-US', {maximumFractionDigits: 0})}</strong> trees/hectare
                        </div>
                    </div>
                    ` : ''}
                    <div class="report-details-row">
            ...
            if (historyData.success && historyData.history.length > 0) {
                console.log('=== STATUS HISTORY DEBUG ===');
                console.log('Total history items:', historyData.history.length);
                
                html += `
                <div class="status-history">
                    <h4>Status History</h4>
                    <div class="status-history-list">
                        ${historyData.history.map((item, itemIndex) => {
                            console.log(`\n--- History Item ${itemIndex} ---`);
                            console.log('Status:', item.status);
                            console.log('Date:', item.date);
                            console.log('admin_attachments (raw):', item.admin_attachments);
                            console.log('admin_attachments type:', typeof item.admin_attachments);
                            console.log('attachment_count:', item.attachment_count);
                            
                            let attachmentsHTML = '';
                            // Parse admin_attachments if it's a string
                            let adminAttachments = item.admin_attachments;
                            
                            if (typeof adminAttachments === 'string' && adminAttachments) {
                                console.log('Attempting to parse admin_attachments string...');
                                try {
                                    adminAttachments = JSON.parse(adminAttachments);
                                    console.log('Successfully parsed admin_attachments:', adminAttachments);
                                } catch (e) {
                                    console.error('Failed to parse admin_attachments:', e);
                                    console.error('String that failed to parse:', adminAttachments);
                                    adminAttachments = [];
                                }
                            } else {
                                console.log('admin_attachments is not a string, using as-is');
                            }
                            
                            console.log('adminAttachments after parsing:', adminAttachments);
                            console.log('Is array?', Array.isArray(adminAttachments));
                            console.log('Array length:', adminAttachments ? adminAttachments.length : 'N/A');
                            
                            if (adminAttachments && Array.isArray(adminAttachments) && adminAttachments.length > 0) {
                                console.log('✓ Attachments will be displayed for this item');
                            } else {
                                console.log('✗ No attachments to display for this item');
                            }
                            
                            if (adminAttachments && Array.isArray(adminAttachments) && adminAttachments.length > 0) {
                                console.log('Building attachments HTML...');
                                attachmentsHTML = `
                                    <div class="admin-attachments-display">
                                        <div class="attachments-header">
                                            <i class='bx bx-paperclip'></i>
                                            <span>Admin Documentation (${item.attachment_count} file${item.attachment_count > 1 ? 's' : ''})</span>
                                        </div>
                                        <div class="admin-attachments-grid">
                                            ${adminAttachments.map((file, index) => {
                                                ...
                                                if (isVideo) {
                                                    return `
                                                        <div class="admin-attachment-item" onclick="openAdminVideo('${file}', '${fileName}')">
                                                    ...
                                                    const imagesOnly = adminAttachments.filter(f => f.match(/\.(jpg|jpeg|png|gif|webp)$/i));
                                                    const imageIndex = imagesOnly.indexOf(file);
                                                    return `
                                                        <div class="admin-attachment-item" onclick='openAdminLightbox(${JSON.stringify(imagesOnly)}, ${imageIndex})'>
                                ...
                                `;
                                console.log('Attachments HTML built successfully');
                                console.log('HTML length:', attachmentsHTML.length);
                            } else {
                                console.log('No attachments HTML will be added');
                            }
            ...
            console.log('Contains admin-attachments-display:', html.includes('admin-attachments-display'));
            
            reportModalContent.innerHTML = html;
            reportModal.style.display = 'block';
            
            // Verify the DOM after insertion
            console.log('=== DOM VERIFICATION ===');
            const attachmentDisplays = reportModalContent.querySelectorAll('.admin-attachments-display');
            console.log('Number of .admin-attachments-display elements found:', attachmentDisplays.length);
            attachmentDisplays.forEach((display, idx) => {
                console.log(`  Display ${idx}:`, display);
                console.log(`  Display ${idx} is visible:`, display.offsetParent !== null);
            });
    ...
    // Use event delegation for view buttons (works with dynamically loaded content)
...
// Admin Lightbox functionality for viewing admin attachments
let currentAdminImageIndex = 0;
let currentAdminImageArray = [];

function openAdminLightbox(imagesArray, startIndex) {
    currentAdminImageArray = imagesArray;
    currentAdminImageIndex = startIndex;
    
    const lightbox = document.getElementById('adminImageLightbox');
    const lightboxImg = document.getElementById('adminLightboxImage');
    const counter = document.getElementById('adminImageCounter');
    const prevBtn = document.getElementById('adminPrevImage');
    const nextBtn = document.getElementById('adminNextImage');
    const caption = document.getElementById('adminLightboxCaption');
    
    if (currentAdminImageArray.length === 0) return;
    
    lightbox.classList.add('active');
    document.body.style.overflow = 'hidden';
    
    updateAdminLightboxImage();
    
    // Navigation button states
    prevBtn.disabled = currentAdminImageIndex === 0;
    nextBtn.disabled = currentAdminImageIndex === currentAdminImageArray.length - 1;
}

function updateAdminLightboxImage() {
    const lightboxImg = document.getElementById('adminLightboxImage');
    const counter = document.getElementById('adminImageCounter');
    const caption = document.getElementById('adminLightboxCaption');
    const prevBtn = document.getElementById('adminPrevImage');
    ...
    lightboxImg.src = currentAdminImageArray[currentAdminImageIndex];
    counter.textContent = `${currentAdminImageIndex + 1} / ${currentAdminImageArray.length}`;
    
    const fileName = currentAdminImageArray[currentAdminImageIndex].split('/').pop();
...
function nextAdminImage() {
    if (currentAdminImageIndex < currentAdminImageArray.length - 1) {
        currentAdminImageIndex++;
        updateAdminLightboxImage();
    }
}

function prevAdminImage() {
    if (currentAdminImageIndex > 0) {
        currentAdminImageIndex--;
        updateAdminLightboxImage();
    }
}

// Admin Video player functionality
function openAdminVideo(videoSrc, fileName) {
    const videoModal = document.getElementById('adminVideoPlayerModal');
    const videoPlayer = document.getElementById('adminVideoPlayer');
    const videoCaption = document.getElementById('adminVideoCaption');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const adminLightbox = document.getElementById('adminImageLightbox');
    if (adminLightbox) {
        adminLightbox.addEventListener('click', function(e) {
            if (e.target === this) {
                closeAdminLightbox();
            }
        });
    ...
    const adminVideoModal = document.getElementById('adminVideoPlayerModal');
    if (adminVideoModal) {
